...
similar to 1564. Put Boxes Into the Warehouse I - on LC
...
def ItemsMax(items, rooms):
    for i in range(1,len(rooms)):
        rooms[i] = min(rooms[i - 1], rooms[i])
    items.sort()
    ans = 0
    for room in reversed(rooms):
       if ans < len(items) and items[ans] <= room:
           ans += 1
    return ans
