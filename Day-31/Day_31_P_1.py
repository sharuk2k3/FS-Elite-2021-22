...
def suitCase(deadends, target) :
    ...
    while Q:
        ...
        if code==target:
            return steps
        for nextcode in bruh(code):
            if nextcode in visited:
                continue
            if nextcode in deadends:
                continue
            visited.add(nextcode)
            Q.append((nextcode,steps+1))
    return -1
    
deadends=list(map(str,input().split()))
target=input()
print(suitCase(deadends, target))
